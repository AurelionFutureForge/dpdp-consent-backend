...
import helmetMiddleware from "./modules/common/middlewares/helmet.middleware";
import corsMiddleware from "./modules/common/middlewares/cors.middleware";
import morganMiddleware from "./modules/common/middlewares/morgan.middleware";
// import limiter from "./middlewares/rateLimiter.middlerware";
import { errorMiddleware, AppError } from "./modules/common/middlewares/errorHandler.middleware";
...
const createApp = async (): Promise<{ app: Express; server: http.Server }> => { 
  ...
  try {
    ...
    const swaggerWebDocs = YAML.load(path.resolve(__dirname, "config/swagger/swagger-dpdp.yaml"));
    app.use("/api-docs", swaggerUI.serve, swaggerUI.setup(swaggerWebDocs));
  } catch (error) {
    console.error("Failed to load Swagger documentation for DPDP API-V1", error);
  }
